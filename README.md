...
- **It is paced to stay unblocked, not to be fast.** Searches ~20s apart, requests to one host ~1.5s apart, enforced by a lock shared across every agent on the machine — so a profile that fans out parallel scouts gets them queued instead of bursting. A suspected throttle escalates the search gap ×4 per strike (20s → 80s → 320s, cap 600s) and persists across invocations, so a fresh process cannot reset it and walk back into the wall. `429` responses honour `Retry-After` up to 120s; beyond that the script reports and stops rather than blocking. `WEB_SEARCH_GAP` / `WEB_FETCH_GAP` override the defaults for interactive one-offs.
