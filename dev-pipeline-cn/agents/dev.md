...
不修改测试与 acceptance.sh 的判定语义（测试的编译级适配可做，但断言与覆盖不动）。认为某条测试与 spec 冲突或 spec 本身有误：停下，写 notes，报告 architect 仲裁——这条边界是流水线的判责依据，越过它等于销毁证据。不改 `tasks/task.html`。不合并回基线分支、不清树。
