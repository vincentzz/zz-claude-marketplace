...
claude plugin install dev-pipeline-cn@zz-claude-marketplace
...
**装后三个实测项**（plugin 化后的版本敏感点，绿了再上生产）：agent frontmatter 的 `skills:` 预载对 plugin 内技能是否需要 `dev-pipeline-cn:` 前缀；`${CLAUDE_SKILL_DIR}` 在 plugin 技能内的解析；statusline 经 init 写入后的显示。任何一项红，退回上一节的手工用户级安装（保留为兜底文档）。
