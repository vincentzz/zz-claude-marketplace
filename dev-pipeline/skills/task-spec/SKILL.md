...
Template: `tasks/specs/_template.html` (`taskctl add` instantiates it automatically); full-marks example: `tasks/specs/_example.html`.
...
3. **Acceptance Criteria** — one row per AC, five columns: number, behavior description, decision command, pass standard (always exit 0), carrying test (left for qa to fill in). AC descriptions use the nouns of 2.1 and the failure classes of 2.3, introducing no new concepts. The count is governed by "covers the target behavior + covers every row of the failure-attribution table", not by hitting a number.
