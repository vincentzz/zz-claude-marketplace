...
5. Launch: `claude --agent architect --model fable` (the frontmatter already says `model: fable`; pinning it again on the command line is insurance for the main session — to make it the default, add `"agent": "architect"` to settings.json). On a machine whose endpoint lacks `fable`/`opus`, read *Running where a different model set is available* below before launching. Consider `--permission-mode acceptEdits`, or keep the git/mvn approval-bypass list in settings; otherwise permission prompts from background subagents keep surfacing and interrupting you.
...
## Running where a different model set is available

The agents' `model:` lines are **tier names**, not a claim about your machine: `fable` for spec and arbitration, `opus` for generation, `sonnet` where cheap is fine. Another machine — a different provider, an org allowlist, a gateway, a laptop on Ollama — backs those tiers with whatever it has, and the environment is where it says so. Claude Code resolves a subagent's model in this order (since 2.1.251; before that the variable came first): per-call parameter > frontmatter > `CLAUDE_CODE_SUBAGENT_MODEL` > main session. Every agent here names a model in its frontmatter, so **`CLAUDE_CODE_SUBAGENT_MODEL` on its own does nothing to this pipeline** — that is the trap. Two ways to make the environment win, both enforced by Claude Code itself, so no agent has to cooperate:

- **Rebind the tiers** — keeps the matrix, works on every version, covers the main session too. `ANTHROPIC_DEFAULT_FABLE_MODEL`, `ANTHROPIC_DEFAULT_OPUS_MODEL`, `ANTHROPIC_DEFAULT_SONNET_MODEL`, `ANTHROPIC_DEFAULT_HAIKU_MODEL` decide what each alias resolves to; point a tier you can't serve at the best model you can, and several aliases may share one target.
- **Flatten to one model** — `CLAUDE_CODE_SUBAGENT_MODEL=<model>` plus `CLAUDE_CODE_SUBAGENT_MODEL_FORCE=1` (Claude Code ≥ 2.1.257) pins every subagent regardless of frontmatter or per-call parameter. It leaves the main session alone: launch with `--model <model>` as well (otherwise the entry agent's `model: fable` applies), or rebind `fable` as above. Architect detects the pin in A0 and switches its model gradient off.

Either set goes in the `env` block of `.claude/settings.local.json` (per project) or `~/.claude/settings.json` (per machine); plain shell exports work too. Measured on 2.1.258 with a probe whose frontmatter said `sonnet`: the variable alone → Sonnet; with `_FORCE=1` → the variable's model; with the Sonnet alias rebound → the rebound model. On 2.1.251–2.1.256 the `_FORCE` flag does not exist yet and only rebinding works. On a non-Anthropic endpoint also set `PIPELINE_PROVIDER=local`, or the budget gate keeps waiting for rate-limit headers that never come. The local-fallback launcher above is the third route: it rewrites the frontmatter into a derived profile, which is the only way to give the *reviewers* a different model from the other roles without editing the plugin.
...
- `CLAUDE_CODE_SUBAGENT_MODEL` is set and subagents still run the frontmatter model: expected on Claude Code ≥ 2.1.251, where frontmatter outranks the variable. Add `CLAUDE_CODE_SUBAGENT_MODEL_FORCE=1` (≥ 2.1.257) or rebind the tiers with `ANTHROPIC_DEFAULT_*_MODEL` — see *Running where a different model set is available*.
