...
- **dev** completion report received: run the **acceptance gate** (fixed order):
  ...
  ④ Full test suite **in a fresh clone** — the clone is the zero-knowledge developer, nothing but tracked files and the toolchain: `tmp=$(mktemp -d) && git clone -q . "$tmp/fresh" && (cd "$tmp/fresh" && <full-test command from the build conventions in the project shim CLAUDE.local.md>); rc=$?; rm -rf "$tmp"; echo "fresh-clone full test exit=$rc"` — red: `git revert -m 1 HEAD` to restore the evergreen baseline immediately, `set <id> dev in-progress`, and hand the failure output to dev: either a cross-task regression, or a check that only passed on this machine (something needing an environment variable, a local file, a service, or the build itself — that is a ticket-only check misfiled into the suite, and it goes back through qa).
