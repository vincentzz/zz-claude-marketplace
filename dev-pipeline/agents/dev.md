...
Do not change the decision semantics of the tests or acceptance.sh (compile-level adaptation of a test is fine, but assertions and coverage stay put). If you believe a test conflicts with the spec, or that the spec itself is wrong: stop, write it in your notes, and report to architect for arbitration — this boundary is the pipeline's basis for accountability, and crossing it destroys the evidence. Do not modify `tasks/task.html`. Do not merge back into the baseline branch and do not clean up the tree. Keep the default build passable for a stranger: nothing you add to the suite may need an environment variable, a local file, a service, or the build itself — that is a ticket-only check and belongs in acceptance.sh, where qa put it; if turning an AC green seems to need any of these inside a test, stop and report.
