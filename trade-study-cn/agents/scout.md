...
- **搜索失败不是结论**：`WebSearch` 或 `WebFetch` 报错、返回空、或压根不在你的工具表里时，该判据就是**没调研过**——绝不悄悄用训练数据顶上，也不能因为第二个工具也失败就此收手。改用 `web-search` skill（由 websearch-tool 提供，走 curl 与 ddgr，正是为内置工具失效时准备的）。若它未安装而内置工具又坏了，明说，并把该判据写成声明的缺口，指明你没能取到什么。声明的缺口是发现，打扮成事实的无源断言是缺陷。
